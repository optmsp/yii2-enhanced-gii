...
class Generator extends \mootensai\enhancedgii\BaseGenerator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [
                ["tableName", "baseControllerClass", "indexWidgetType", "db"],
                "required",
            ],
            [
                ["tableName"],
            ...
            [
                [
                    "relationsExpandableRelatedColumns",
                    "manyRelationsHiddenFieldTweak",
                ],
                "string",
            ],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            "tableName" => 'This is the name of the DB table that the new ActiveRecord class is associated with, e.g. <code>post</code>.
                ...
                class.',
            "nameAttribute" =>
                "This is the (set) of name column that you use to show as label, " .
            ...
            "nsSearchModel" =>
                "This is the namespace of the search model class to be generated, e.g., <code>app\models</code>",
            ...
            "generateMigrations" => 'This indicates whether the generator should generate migrations based on
                table structure.',
            "optimisticLock" =>
                "This indicates whether the generator should generate optimistic lock feature for Model. " .
                "Enter this field with optimistic lock column name. " .
                "Empty this field if you want to disable this feature.",
            "createdAt" =>
                "This indicates whether the generator should generate Timestamp Behaviors feature for Model. " .
            ...
            "updatedAt" =>
                ...
                'Empty "Created At" & "Updated At" field if you want to disable this feature.',
            "timestampValue" =>
                'This will generate the </code>value</code> configuration entry for Timestamp Behaviors.  e.g., <code>new Expression(\'NOW()\')</code>',
            "createdBy" =>
                "This indicates whether the generator should generate Blameable Behaviors feature for Model. " .
            ...
            "updatedBy" =>
                ...
                'Empty "Created By" & "Updated By" field if you want to disable this feature.',
            "blameableValue" =>
                'This will generate the </code>value</code> configuration entry for Blameable Behaviors.  e.g., <code>new Expression(\'NOW()\')</code>',
            "UUIDColumn" =>
                "This indicates whether the generator should generate UUID Behaviors feature for Model. " .
                "Enter this field with UUID column name. " .
                'Empty "UUID Column" field if you want to disable this feature.',
            "deletedBy" =>
                "This indicates whether the generator should generate Soft Delete feature for Model. " .
            ...
            "deletedAt" =>
                ...
                'Empty "Deleted By" & "Deleted At" field if you want to disable this feature.',
            "nsController" =>
                "This is the namespace of the Controller class to be generated, e.g., <code>app\controllers</code>",
            ...
            "expandable" =>
                "Set the generator to generate expandable/collapsible row for related at index",
            "cancelable" =>
                "Set the generator to generate cancel button to return to grid view at form",
            "pdf" =>
                "Set the generator to generate printable PDF generator at view",
    ...
    private function explodeCmdLineParameter(
        string $value,
        int $expectedGroups,
        string $rowSep = ";",
        string $groupSep = ":",
    ...
    public function generate()
    {
        ...
        $this->nameAttribute = $this->nameAttribute
            ? explode(",", str_replace(" ", "", $this->nameAttribute))
            : [$this->nameAttribute];
        $this->hiddenColumns = $this->hiddenColumns
            ? explode(",", str_replace(" ", "", $this->hiddenColumns))
            : [$this->hiddenColumns];

        $this->skippedColumns = $this->skippedColumns
            ? explode(",", str_replace(" ", "", $this->skippedColumns))
            : [$this->skippedColumns];
        $this->skippedColumnsInCreate = $this->skippedColumnsInCreate
            ? explode(",", str_replace(" ", "", $this->skippedColumnsInCreate))
            : [$this->skippedColumnsInCreate];
        $this->skippedColumnsInUpdate = $this->skippedColumnsInUpdate
            ? explode(",", str_replace(" ", "", $this->skippedColumnsInUpdate))
            : [$this->skippedColumnsInUpdate];
        $this->skippedIndexColumns = $this->skippedIndexColumns
            ? explode(",", str_replace(" ", "", $this->skippedIndexColumns))
            : [$this->skippedIndexColumns];
        $this->skippedRelations = $this->skippedRelations
            ? explode(",", str_replace(" ", "", $this->skippedRelations))
            : [$this->skippedRelations];
        $this->manyRelationsAllowedInEdit = $this->manyRelationsAllowedInEdit
            ? explode(
                ",",
                str_replace(" ", "", $this->manyRelationsAllowedInEdit)
            )
            : [$this->manyRelationsAllowedInEdit];
        $this->manyRelationsAllowedInView = $this->manyRelationsAllowedInView
            ? explode(
                ",",
                str_replace(" ", "", $this->manyRelationsAllowedInView)
            )
            : [$this->manyRelationsAllowedInView];

        $this->allowDeleteWithRelatedList = $this->allowDeleteWithRelated
            ? explode(",", str_replace(" ", "", $this->allowDeleteWithRelated))
            : [$this->allowDeleteWithRelated];
        $this->allowDeleteWithRelatedList = array_filter(
            $this->allowDeleteWithRelatedList
        );

        $this->detailViewFormatCodeList = $this->explodeCmdLineParameter(
            $this->detailViewFormatCode,
            2
        );
        $this->detailViewValueCodeList = $this->explodeCmdLineParameter(
            $this->detailViewValueCode,
            2,
            "!",
            "#",
            "?"
        ...
        $this->tweakRules = $this->explodeCmdLineParameter(
            $this->manyRelationsEditQueryTweak,
            3
        );
        $this->relationsExpandableRelatedColList = $this->explodeCmdLineParameter(
            $this->relationsExpandableRelatedColumns,
            2
        );
        $this->manyRelationsHiddenFieldTweakList = $this->explodeCmdLineParameter(
            $this->manyRelationsHiddenFieldTweak,
            3,
            ";",
            "#"
        );

        $this->manyRelationsEditFieldList = $this->explodeCmdLineParameter(
            $this->manyRelationsEditFields,
            2
        );
        $this->manyRelationsViewFieldList = $this->explodeCmdLineParameter(
            $this->manyRelationsViewFields,
            2
        );

        $this->skippedColumns = array_filter($this->skippedColumns);
        $this->skippedColumnsInCreate = array_filter(
            $this->skippedColumnsInCreate
        );
        $this->skippedColumnsInUpdate = array_filter(
            $this->skippedColumnsInUpdate
        );
        $this->skippedIndexColumns = array_filter($this->skippedIndexColumns);
        $this->skippedRelations = array_filter($this->skippedRelations);
        $this->manyRelationsAllowedInEdit = array_filter(
            $this->manyRelationsAllowedInEdit
        );
        $this->manyRelationsAllowedInView = array_filter(
            $this->manyRelationsAllowedInView
        );

        $this->editFormDepFieldList = $this->explodeCmdLineParameter(
            $this->editFormDepFields,
            2
        );
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if (strpos($this->tableName, "*") !== false) {
                ...
                $modelClassName = !empty($this->modelClass)
                    ? $this->modelClass
                    : Inflector::id2camel($tableName, "_");
                $controllerClassName = !empty($this->controllerClass)
                    ? $this->controllerClass
                    : $modelClassName . "Controller";
            }
            ...
            $this->controllerClass =
                $this->nsController . "\\" . $controllerClassName;
            $isTree = !array_diff(
                self::getTreeColumns(),
            ...
            if ($this->generateSearchModel && !$isTree) {
                ...
                $files[] = new CodeFile(
                    ...
                    $this->render("search.php", [
                        "relations" => isset($relations[$tableName])
                            ? $relations[$tableName]
                            : [],
                    ])
                );
            ...
            foreach (scandir($templatePath) as $file) {
                ...
                if (
                    $file === "_formrefone.php" ||
                    $file === "_formrefmany.php" ||
                    $file === "_datarefone.php" ||
                    $file === "_datarefmany.php" ||
                    $file === "_expand.php" ||
                    $file === "_detail.php" ||
                    $file === "_data.php" ||
                    $file === "saveAsNew.php" ||
                ...
                if (
                    !$isTree &&
                    ($file === "indexNested.php" || $file === "_formNested.php")
                ) {
                    continue;
                }
                if (
            ...
            if ($this->expandable) {
                $files[] = new CodeFile(
                    "$viewPath/_expand.php",
                    $this->render("views/_expand.php", [
                        "relations" => isset($relations[$tableName])
                            ? $relations[$tableName]
                            : [],
                    ])
            ...
            if ($this->saveAsNew) {
                $files[] = new CodeFile(
                    "$viewPath/saveAsNew.php",
                    $this->render("views/saveAsNew.php", [
                        "relations" => isset($relations[$tableName])
                            ? $relations[$tableName]
                            : [],
                    ])
            ...
            if (isset($relations[$tableName]) && !$isTree) {
                if ($this->expandable) {
                    $files[] = new CodeFile(
                        "$viewPath/_detail.php",
                        $this->render("views/_detail.php", [
                            "relations" => isset($relations[$tableName])
                                ? $relations[$tableName]
                                : [],
                        ])
                ...
                foreach ($relations[$tableName] as $name => $rel) {
                    if (
                        $rel[self::REL_IS_MULTIPLE] &&
                        isset($rel[self::REL_TABLE]) &&
                        !in_array($name, $this->skippedRelations)
                    ) {
                        $files[] = new CodeFile(
                            "$viewPath/_form{$rel[self::REL_CLASS]}.php",
                            $this->render("views/_formrefmany.php", [
                                "relations" => isset($relations[$tableName])
                                    ? $relations[$tableName][$name]
                                    : [],
                            ])
                        );
                        if ($this->expandable) {
                            $files[] = new CodeFile(
                                "$viewPath/_data{$rel[self::REL_CLASS]}.php",
                                $this->render("views/_datarefmany.php", [
                                    "relName" => $name,
                                    "relations" => isset($relations[$tableName])
                                        ? $relations[$tableName][$name]
                                        : [],
                        ...
                        isset($rel[self::REL_IS_MASTER]) &&
                        !$rel[self::REL_IS_MASTER] &&
                        !in_array($name, $this->skippedRelations)
                    ) {
                        $files[] = new CodeFile(
                            "$viewPath/_form{$rel[self::REL_CLASS]}.php",
                            $this->render("views/_formrefone.php", [
                                "relName" => $name,
                                "relations" => isset($relations[$tableName])
                                    ? $relations[$tableName][$name]
                                    : [],
                        ...
                        if ($this->expandable) {
                            $files[] = new CodeFile(
                                "$viewPath/_data{$rel[self::REL_CLASS]}.php",
                                $this->render("views/_datarefone.php", [
                                    "relName" => $name,
                                    "relations" => isset($relations[$tableName])
                                        ? $relations[$tableName][$name]
                                        : [],
        ...
        $this->nameAttribute = is_array($this->nameAttribute)
            ? implode(", ", $this->nameAttribute)
            : "";
        $this->hiddenColumns = is_array($this->hiddenColumns)
            ? implode(", ", $this->hiddenColumns)
            : "";
        $this->skippedColumns = is_array($this->skippedColumns)
            ? implode(", ", $this->skippedColumns)
            : "";
        $this->skippedRelations = is_array($this->skippedRelations)
            ? implode(", ", $this->skippedRelations)
            : "";
    ...
    public function getViewPath()
    {
        if (empty($this->viewPath)) {
            ...
            return Yii::getAlias(
                $this->viewPath . "/" . $this->getControllerID()
            );
        }
    }
    ...
    public function getNameAttribute()
    {
        foreach ($this->tableSchema->getColumnNames() as $name) {
            foreach ($this->nameAttribute as $nameAttr) {
                if (
                    !strcasecmp($name, $nameAttr) ||
                    !strcasecmp($name, $this->tableSchema->fullName)
                ) {
                    return $name;
                }
            }
        ...
        $pk = empty($this->tableSchema->primaryKey)
            ? $this->tableSchema->getColumnNames()[0]
            : $this->tableSchema->primaryKey[0];
    ...
    public function getNameAttributeFK($tableName)
    {
        ...
        $pk = empty($tableSchema->primaryKey)
            ? $tableSchema->getColumnNames()[0]
    ...
    public function generateFK($tableSchema = null)
    {
        ...
        if (isset($this->relations[$tableSchema->fullName])) {
            foreach (
                $this->relations[$tableSchema->fullName]
                as $name => $relations
            ) {
                foreach ($tableSchema->foreignKeys as $value) {
                    if (
                        isset($relations[self::REL_FOREIGN_KEY]) &&
                        $relations[self::REL_TABLE] ==
                            $value[self::FK_TABLE_NAME]
                    ) {
                        if (
                            $tableSchema->fullName ==
                                $value[self::FK_TABLE_NAME] &&
                            $relations[self::REL_IS_MULTIPLE]
                        ) {
                            // In case of self-referenced tables (credit to : github.com/iurijacob)
                        } else {
     ...
     * @return string
     */
    public function generateDetailViewField(
        $attribute,
        $fk,
        $tableSchema = null
    ) {
        ...
        if (
            $tableSchema === false ||
            !isset($tableSchema->columns[$attribute])
        ) {
            if (
        ...
        if (array_key_exists($attribute, $fk)) {
            ...
            if (
                ArrayHelper::keyExists(
                    $attribute,
                    $this->detailViewValueCodeList
                )
            ) {
                $valueLine =
                    "'value' => " .
                    $this->detailViewValueCodeList[$attribute][0] .
                    ",";
            } else {
                $valueLine = null;
            }
     ...
     * @return string
     */
    public function generateGridViewFieldIndex(
        $attribute,
        $fk,
        $tableSchema = null
    ) {
        ...
        if (array_key_exists($attribute, $fk) && $attribute) {
            ...
            $id =
                "grid-" .
                Inflector::camel2id(
                    StringHelper::basename($this->searchModelClass)
                ) .
                "-" .
                $attribute;
            //            $modelRel = $rel[2] ? lcfirst(Inflector::pluralize($rel[1])) : lcfirst($rel[1]);
            if ($column->allowNull) {
                ...
                if (
                    $this->generateSearchModel &&
                    isset($this->webUserColName)
                ) {
                    $webUserColNameText =
                        "andWhere(['" .
                        $this->webUserColName .
                ...
                'filter' => \\yii\\helpers\\ArrayHelper::map(\\$this->nsModel\\$rel[1]::find()->" .
                    $webUserColNameText .
     ...
     * @return string
     */
    public function generateTabularFormField(
        $attribute,
        $fk,
        $tableSchema = null
    ) {
        ...
        if ($column->autoIncrement) {
            ...
            $hasWebUserColColumn = array_key_exists(
                $this->webUserColName,
                $fkTableSchema->columns
            );
            ...
            $output =
                ...
                'options' => [
                    'data' => \\yii\\helpers\\ArrayHelper::map($fkClassFQ::find()->" .
                $webUserColNameText .
                $tweakFilter .
                "asArray()->all(), '{$rel[self::REL_PRIMARY_KEY]}', '$labelCol'),
            ...
            if (
                is_array($column->enumValues) &&
                count($column->enumValues) > 0
            ) {
                $dropDownOptions = [];
                foreach ($column->enumValues as $enumValue) {
                    $dropDownOptions[$enumValue] = Inflector::humanize(
                        $enumValue
                    );
                }
        ...
        'options' => [
            ...
            if (array_key_exists($attribute, $this->editFormDepFieldList)) {
                ...
                $controllerActionName =
                    \yii\helpers\Inflector::camel2id($attribute) . "-list";
                ...
                $output =
                    "\$form->field($model, '$attribute')->widget(\\kartik\\widgets\\Select2::classname(), [
                    'data' => \\yii\\helpers\\ArrayHelper::map($fkClassFQ::find()->" .
                    $webUserColNameText .
                    $tweakFilter .
                    "asArray()->all(), '$rel[4]', '$labelCol'),
                    'options' => [
                        'placeholder' => '--INPUT--',
                        'id' => 'field_$attribute',
                    ],
    ...
    public function generateActionParamComments()
    {
        ...
        if (count($pks) === 1) {
            ...
            foreach ($pks as $pk) {
                $params[] =
                    "@param " . $table->columns[$pk]->phpType . ' $' . $pk;
            }
        ...
        foreach ($this->getColumnNames() as $name) {
            if (isset($attributeLabels[$name])) {
                ...
                if (!strcasecmp($name, "id")) {
                    ...
                    if (
                        !empty($label) &&
                        substr_compare($label, " id", -3, 3, true) === 0
                    ) {
