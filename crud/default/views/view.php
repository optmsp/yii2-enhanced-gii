...
<?php
$modelClassName = $generator->generateString(
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="col-lg-12">
                ...
                <?= "<?= " ?>
                <?= "
                 Html::a('<i class=\"fa glyphicon glyphicon-hand-up\"></i> ' . " .
                    $generator->generateString("PDF") .
                    ",
                    ['pdf', $urlParams],
                    [
                        ...
                        'title' => " .
                    $generator->generateString(
                        "Will open the generated PDF file in a new window"
                    ) .
                    "
                    ]
                )?>\n" ?>
                ...
                <?php if ($generator->saveAsNew): ?>
                    <?= "            <?= Html::a(" .
                        $generator->generateString("Save As New") .
                        ", ['save-as-new', " .
                ...
                <?php if ($tableSchema === false) {
                    ...
                    foreach ($tableSchema->getColumnNames() as $attribute) {
                        if (!in_array($attribute, $generator->skippedColumns)) {
                            echo "        " .
                                $generator->generateDetailViewField(
                                    $attribute,
                                    $fk,
                                    $tableSchema
                                );
        ...
        <div class="row">
                ...
                <?=
                    ...
                    ? Html::a(" .
                    $generator->generateString(
                        "Create Related $generator->extraCreateButton"
                    ) .
                    ",
                        ['" .
                    \yii\helpers\Inflector::camel2id(
                        $generator->extraCreateButton
                    ) .
                    "/create', 'id' => \$model->id],
...
<?php // this is for sub forms for relations
foreach ($generator->manyRelationsAllowedInView as $name):
    ...
    if(<?= $ifStmt ?>)
    {
        $gridColumn<?= $rel[Generator::REL_CLASS] ?> = [
            ...
            <?php
            $relTableSchema = $generator
                ->getDbConnection()
                ->getTableSchema($rel[Generator::REL_TABLE]);
            ...
            if ($tableSchema === false) {
                ...
                foreach ($relTableSchema->getColumnNames() as $attribute) {
                    // xx
                    if (
                        $generator->relationsShowSubDetailsColCount > 0 &&
                    ...
                    if (!in_array($attribute, $generator->skippedColumns)) {
                        ...
                        //echo "\t\t\t" . $attribute . "',\n";
                        echo "\t\t\t" .
                            $generator->generateColumnFormat($attribute) .
                            "',\n";
                    }
                }
            } else {
                ...
                foreach ($relTableSchema->getColumnNames() as $attribute) {
                    ...
                    $allowedColList = \yii\helpers\ArrayHelper::getValue(
                        $generator->manyRelationsViewFieldList,
                        $relClassName,
                        []
                    );
                    ...
                    if (
                        ...
                        ($allowColAllowed ||
                            in_array($attribute, $allowedColList))
                    ) {
                        $count++;
                        // xxx
                        echo "\t\t\t" .
    ...
    <?= "<?php\n" ?>
        ...
        echo Gridview::widget([
            ...
            'pjax' => true,
            'pjaxSettings' => ['options' => ['id' => 'kv-pjax-container-<?= Inflector::camel2id(
                $rel[Generator::REL_TABLE]
            ) ?>']],
            'panel' => [
                <?php if ($isEditRelation) {
                    echo "'type' => GridView::TYPE_PRIMARY,\n";
                    $urlParms =
                        "'update-" .
                        Inflector::camel2id(Inflector::singularize($name)) .
                        "', " .
                        $generator->generateUrlParams();
...
<?php
        ...
        elseif (
        empty($rel[Generator::REL_IS_MULTIPLE]) &&
        $generator->relationsShowSubDetailsInView
    ): ?>
    <div class="row">
        <h4><?=
        $rel[Generator::REL_CLASS]
        "<?= "
...
foreach ($relTableSchema->getColumnNames() as $attribute) {
    ...
    if ($relTableSchema === false) {
        ...
        if (!in_array($attribute, $generator->skippedColumns)) {
            echo "        " .
                $generator->generateDetailViewField($attribute, $fkRel);
        }
    }
