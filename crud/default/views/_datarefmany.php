...
<?php if ($tableSchema === false) {
    ...
    $allowedColList = ArrayHelper::getValue(
        $generator->relationsExpandableRelatedColList,
        $className
    );
    $allowedColList[] = ArrayHelper::getValue(
...
<?php if (
    isset($generator->relationsExpandableRelatedActions) &&
    strlen($generator->relationsExpandableRelatedActions) > 0
): ?>
        [
            'class' => 'yii\grid\ActionColumn',
<?php echo "\t\t\t'template' => '" .
    $generator->relationsExpandableRelatedActions .
    "', \n"; ?>
            'controller' => '<?= \yii\helpers\Inflector::camel2id(
