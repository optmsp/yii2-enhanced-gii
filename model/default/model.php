...
use common\components\activerecord\behaviors\UUIDBehavior;
<?php endif; ?>
...
'is_deleted' => 0,
'<?= $generator->deletedBy ?>' => <?= empty($generator->deletedByValueRestored)
    ? 0
    : $generator->deletedByValueRestored ?>,
<?php if ($generator->deletedAt): ?>
'<?= $generator->deletedAt ?>' => <?= empty($generator->deletedAtValueRestored)
    ? 0
    : $generator->deletedAtValueRestored ?>,
<?php endif; ?>
];
}
<?php endif; ?>
...
/**
* @inheritdoc
* @return <?= $queryClassFullName ?> the active query used by this AR class, returns both deleted and non-deleted.
*/
public static function findAny()
{
return new <?= $queryClassFullName ?>(get_called_class());
}
...
/**
* @inheritdoc
* @return <?= $queryClassFullName ?> the active query used by this AR class. Returns only soft-deleted entries.
*/
public static function findDeleted()
{
<?php if ($generator->deletedBy): ?>
$query = new <?= $queryClassFullName ?>(get_called_class());
return $query->where(['<?= $tableName ?>.is_deleted' => <?= 1 ?>]);
<?php else: ?>
$query = new <?= $queryClassFullName ?>(get_called_class());
...
return sprintf('%s (%d)', $this->getAttribute('name'), $this->id);
} else {
